...
# "Docx" is the engine for reading and writing Word documents.
...
def main():
    ...
    directory, merges all the .docx files there, and counts its words, characters
    and \"standard pages\"."""
    full_path = get_path()
...
def merger(full_path):
    ...
    master = Document(f"{full_path}\\{folder[0]}")
    composer = Composer(master)
    for i in range(1, len(folder)):
        document = Document(f"{full_path}\\{folder[i]}")
        composer.append(document)
    composer.save(f"{full_path}\\merged_{date.today()}_{folder[0]}")
    new_doc = Document(f"{full_path}\\merged_{date.today()}_{folder[0]}")
